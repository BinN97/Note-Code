...
# 区块链
背景：
一组验证者（矿工）共同维护一份分布式数字账本，每个人自己保留一份独立的账本副本，在受到新的交易时按照事先预定的规则对自己的账本进行更新。目标是使得存在少数恶意验证者的情况下，账本仍然正确无误，在任意时刻诚实方都能看到相同的账本。
...
+ 确定性：从genesis状态重播的事物，拥有相同的结果状态。
+ 共识安全：由状态机复制的每个诚实节点状态应该相同，节点接受到交易，会进行验证，多于2/3的验证者通过代表交易有效。当存在不多于1/3的拜占庭节点，系统是安全的。

分为三层
+ Applications：负责更新事物的状态，更新交易状态
+ Consensus： 保证节点对系统状态达成一致
+ Networking：交易和共识相关消息传播

# COSMOS-更为广泛的区块链生态系统
## 1.0 比特币
点对点数字货币，使用POW共识算法，第一个分布式应用程序，具有较大的潜力。

开发去中心化应用方案：
  + 分叉比特币代码库
  + 建立在其之上
问题：
  + 比特币的代码块耦合性严重，三层耦合在一起；
  + 脚本语言有限，也不用户友好；
  
## 2.0 以太坊
提出一个单一的区块链，在其上可以构建任意的去中心化区块链应用，使用<b>以太坊虚拟机</b>替换<b>应用层</b>来实现，能够处理智能合约，任何人都可以进行开发。

局限性：
  + 扩展性（Scalability）：使用POW共识过程，交易速度（15笔/s）限制了应用程序，同时以太坊DAPP争夺区块链的有限资源
  + 可用性（Usability）：授于开发人员的灵活性较低，EVM是一个需要容纳所有的用户场景的沙箱，对常用场景进行优化，导致开发人员设计用程序要兼顾效率和设计。同时受限于指定的编程语言，且代码无法自动执行
  + 主权（Sovereignty)：由于基础环境相同，应用程序都会在主权方面受到限制。本质上，这会导致两层治理：应用程序及底层环境治理，前者受到后者限制。如果应用程序存在错误，无法进行任何操作，只能是经过以太坊平台本身进行治理（DAO）
...
以太坊借助虚拟机区块链，来简化DApp的开发流程，通过智能合约自行部署逻辑。但是区块链本身的开发并未简化，就像比特币一样，Ethereum仍然是耦合性较高的系统，难以分叉、定制，而这也是创建Tendermint的背景。
...
+ 即时最终确定性（Instant finality）：只要超过2/3的验证者是诚实的，就不会出错，用户可确保区块一旦创建交易即有效（而比特币、以太坊需要等待一段时间才会确认）
...
通过多个hub连接
![](https://i.loli.net/2020/11/22/uLUck2YmI7pqXRW.png)
